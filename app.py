    ...
    if not stats_data:
        ...
        with col_prev:
            if "def_card_bytes" in st.session_state:
                st.image(BytesIO(st.session_state["def_card_bytes"]), caption="Defensive Stats Card", use_container_width=True)
                st.download_button(
                    "Download PNG",
    ...
    if logs:
        ...
        with col_prev2:
            if "player_card_bytes" in st.session_state:
                st.image(BytesIO(st.session_state["player_card_bytes"]), caption="Player Log Card", use_container_width=True)
                st.download_button(
                    "Download PNG",
        ...
        if team_odds:
            ...
            with col_prev3:
                if "odds_card_bytes" in st.session_state:
                    st.image(BytesIO(st.session_state["odds_card_bytes"]), caption="Odds Card", use_container_width=True)
                    st.download_button(
                        "Download PNG",
