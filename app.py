    ...
    if not stats_data:
        ...
        with right_col:
            ...
            if "def_card_bytes" in st.session_state:
                st.image(Image.open(BytesIO(st.session_state["def_card_bytes"])), use_container_width=True)
                st.download_button(
                    "DOWNLOAD PNG",
...
with tab_player:
    ...
    with right_col2:
        ...
        if "player_card_bytes" in st.session_state:
            st.image(Image.open(BytesIO(st.session_state["player_card_bytes"])), use_container_width=True)
            st.download_button(
                "DOWNLOAD PNG",
...
with tab_odds:
    ...
    with right_col3:
        ...
        if "odds_card_bytes" in st.session_state:
            st.image(Image.open(BytesIO(st.session_state["odds_card_bytes"])), use_container_width=True)
            st.download_button(
                "DOWNLOAD PNG",
